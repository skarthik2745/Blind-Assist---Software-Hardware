...
import { MessageSquare, AlertCircle, Pill, Camera, LogOut, XCircle, Users, Clock, Save, Activity, ChevronUp, ChevronDown, Radio } from 'lucide-react';
import { voiceService } from '../services/voiceService';
import { fetchBlynkPins, BlynkPinData } from '../services/blynkService';
...
export default function PatientDashboard() {
  ...
  // =====================================================
  const prevBlynkRef = useRef<BlynkPinData | null>(null);
  ...
  useEffect(() => {
    ...
    const pollBlynk = async () => {
      ...
      isFetching = true;
      try {
        const data = await fetchBlynkPins();
        if (!isMounted) return;

        if (data) {
          setIsHardwareOnline(true);

          if (!prevBlynkRef.current) {
            // First read on page load: record baseline without triggering
            prevBlynkRef.current = { ...data };
            return;
          }

          const prev = prevBlynkRef.current;
          const pins: Array<keyof BlynkPinData> = ['V0', 'V1', 'V2', 'V3'];

          for (const pin of pins) {
            const newVal = data[pin];
            const oldVal = prev[pin];

            if (newVal !== oldVal) {
              prev[pin] = newVal;
              const now = Date.now();
              const lastChange = lastChangeTimeRef.current[pin] || 0;
              const timeSinceLast = now - lastChange;
              lastChangeTimeRef.current[pin] = now;

              // If newVal is 1, it's an intentional activation/press.
              // If newVal is 0, only trigger if timeSinceLast >= 1500ms (intentional second press/switch-off, not a momentary release).
              const shouldTrigger = newVal === 1 || timeSinceLast >= 1500;

              if (shouldTrigger) {
                console.log(`Blynk hardware event: pin ${pin} -> ${newVal}`);
                if (pin === 'V0') {
                  actionHandlersRef.current.triggerSOS();
                } else if (pin === 'V1') {
                  actionHandlersRef.current.triggerVoiceRecording();
                } else if (pin === 'V2') {
                  actionHandlersRef.current.triggerObjectDetection();
                } else if (pin === 'V3') {
                  actionHandlersRef.current.triggerMedicineReminder();
                }
              }
            }
          }
        ...
        console.error('Blynk polling error:', err);
      } finally {
        isFetching = false;
      }
    };

    const intervalId = setInterval(pollBlynk, 800);
    pollBlynk();
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div className="flex items-center gap-4">
            ...
            {isHardwareOnline && (
              <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 border border-emerald-300 shadow-sm" title="Blynk hardware connected">
