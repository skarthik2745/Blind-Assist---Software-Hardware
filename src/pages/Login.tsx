    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-blue-100 flex flex-col items-center justify-center p-4">
      ...
      <div className="mt-8 flex flex-col items-center gap-4">
        <button
          ...
          className="px-8 py-3 bg-green-600 hover:bg-green-700 text-white text-lg font-semibold rounded-lg shadow-md active:scale-95 transition-all"
        >
        ...
        <div className="flex flex-wrap gap-3 justify-center mt-2">
          <button
            onClick={async () => {
              const success = await signIn('shankar');
              if (success) {
                navigate('/patient-dashboard');
              }
            }}
            className="px-4 py-2 bg-white hover:bg-blue-50 text-blue-800 text-sm font-semibold rounded-lg border border-blue-300 shadow-sm transition-all"
          >
            Demo Patient: John (voice: "shankar")
          </button>
          <button
            onClick={async () => {
              const success = await signIn('doctor');
              if (success) {
                navigate('/doctor-dashboard');
              }
            }}
            className="px-4 py-2 bg-white hover:bg-slate-50 text-slate-800 text-sm font-semibold rounded-lg border border-slate-300 shadow-sm transition-all"
          >
            Demo Doctor: Dr. Sarah Smith (voice: "doctor")
          </button>
        </div>
      </div>
    </div>
